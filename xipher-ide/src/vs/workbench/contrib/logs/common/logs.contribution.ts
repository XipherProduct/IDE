...
class LogOutputChannels extends Disposable implements IWorkbenchContribution {
	...
	private registerLogChannel(logger: ILoggerResource): void {
		...
		const remoteLogger = existingChannel && isSingleSourceOutputChannelDescriptor(existingChannel) && existingChannel.source.resource.scheme === Schemas.alaskacodeRemote ? this.loggerService.getRegisteredLogger(existingChannel.source.resource) : undefined;
		if (remoteLogger) {
			this.deregisterLogChannel(remoteLogger);
		}
		const hasToAppendRemote = existingChannel && logger.resource.scheme === Schemas.alaskacodeRemote;
