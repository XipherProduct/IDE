...
export async function findInFilesCommand(accessor: ServicesAccessor, _args: IFindInFilesArgs = {}) {
	...
	if (Object.keys(_args).length !== 0) {
		...
		const filteredActiveWorkspaceRootUri = activeWorkspaceRootUri?.scheme === Schemas.file || activeWorkspaceRootUri?.scheme === Schemas.alaskacodeRemote ? activeWorkspaceRootUri : undefined;
