...
class AlaskaStatusbarContribution extends Disposable implements IWorkbenchContribution {
	...
	private formatStatus(): string {
		...
		if (state.status === 'signed-in') {
			return '$(circle-filled) xipher · online';
		}
		return '$(circle-outline) xipher · offline';
	}
	...
	private formatUsageTooltip(usage: IAlaskaUsage): string {
		const weekUsed = usage.credits_used ?? 0;
		const weekLimit = usage.weekly_budget ?? usage.limit ?? 0;
		const fiveHUsed = usage.credits5h_used ?? 0;
		const fiveHLimit = usage.credits5h_limit ?? 0;
		const fmt = (used: number, limit: number) => `${Math.round(used * 10) / 10} / ${limit || '∞'}`;
		const tokensTotal = (usage.input ?? 0) + (usage.output ?? 0);
		const lines = [
			// allow-any-unicode-next-line
			`Тариф: ${usage.plan_label ?? usage.plan}`,
			// allow-any-unicode-next-line
			`Кредиты за 5 часов: ${fmt(fiveHUsed, fiveHLimit)}`,
			// allow-any-unicode-next-line
			`Кредиты за неделю: ${fmt(weekUsed, weekLimit)}`,
			// allow-any-unicode-next-line
