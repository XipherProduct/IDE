...
export interface IAlaskaModel {
	...
	readonly credit_multiplier?: number;
...
export interface IAlaskaUsage {
	...
	readonly credits5h_used?: number;
	readonly credits5h_limit?: number;
}
...
export class AlaskaChatService extends Disposable implements IAlaskaChatService {
	...
	async models(token: CancellationToken): Promise<IAlaskaModelsResponse> {
		...
		if (res.status === 401) {
			...
			throw new Error('Your session expired. Run `Xipher IDE: Sign In` from the command palette to re-authenticate.');
		}
		if (!res.ok) {
	...
	async *stream(req: IAlaskaChatRequest, token: CancellationToken): AsyncIterable<IAlaskaChatEvent> {
		...
		const creds = await this.authService.getCredentials();
		if (!creds) {
			yield { kind: 'error', message: 'Not signed in. Click Sign in to authorize with Xipher IDE.' };
			return;
		}
		const userId = this.authService.state.user?.id;
		if (!userId) {
		...
		if (res.status === 401) {
			try { await this.authService.signOut(); } catch { }
			yield { kind: 'error', message: 'Your session expired. Run `Xipher IDE: Sign In` from the command palette to re-authenticate.' };
			return;
		}
		if (!res.ok) {
			const text = await res.text().catch(() => '');
...
function renderContextAsSystemMessage(ctx: IAlaskaCodeContext | undefined, toolsEnabled: boolean, replyMode?: 'snippet'): string {
	...
	lines.push(
		THINKING_CONTRACT,
		THINKING_FEW_SHOT,
		'You are Xipher IDE, the user\'s in-IDE coding assistant. Be concise: write code, not exposition.',
	);
	if (toolsEnabled) {
		lines.push(
			...
			'NEVER emit an empty assistant message. If every planned change is already on disk, or you have nothing else to do, you MUST end your turn with at least one sentence of visible text. The IDE renders empty replies as "No response from Xipher IDE." and the user reads that as a crash.',
			...
			'BROWSER PANEL — alaska_open_browser opens a URL in the IDE\'s built-in browser panel (Simple Browser) so the user SEES the live rendered page next to their code, exactly like Cursor\'s in-editor browser. Use it when the user wants to LOOK at a page live rather than have you read it: after you start a dev server (open http://localhost:3000 / :5173 / :8080 so they can preview their app), when they ask you to "open"/"show"/"preview" a site or design, or to display docs they want to browse themselves. localhost/dev-server URLs are allowed here (they render in the user\'s own webview) even though alaska_web_fetch blocks private hosts. This is complementary to alaska_web_fetch: use alaska_web_fetch to READ page content INTO your own context, use alaska_open_browser to SHOW a page to the user. It is read-only and needs no approval. A common flow after building a web app: run the dev server via alaska_run_command, then alaska_open_browser its localhost URL so the user immediately sees the result.',
		);
	} else {
		lines.push(
