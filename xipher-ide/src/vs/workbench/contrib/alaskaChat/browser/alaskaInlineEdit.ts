...
export class AlaskaInlineEditController extends Disposable implements IEditorContribution {
	...
	start(): void {
		...
		if (selection.isEmpty()) {
			this.notificationService.notify({ severity: Severity.Info, message: 'Xipher IDE: select something first.' });
			return;
		}
		this.cleanupSession();
...
class PromptWidget implements IContentWidget {
	...
	constructor(
		...
		hint.textContent = 'Xipher IDE · Ctrl+K';
	...
	setStreaming(streaming: boolean): void {
		...
		this.hintEl.textContent = streaming ? 'Streaming…' : 'Xipher IDE · Ctrl+K';
	}
...
class AlaskaInlineEditAction extends EditorAction {
	constructor() {
		super({
			...
			label: localize2('alaska.inlineEdit.label', 'Xipher IDE: Edit Selection'),
