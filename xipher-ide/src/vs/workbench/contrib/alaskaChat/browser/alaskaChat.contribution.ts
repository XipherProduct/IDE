...
import { AlaskaGoalService } from './alaskaGoalService.js';
import { IAlaskaReminderService } from '../common/alaskaReminder.js';
import { AlaskaReminderService } from './alaskaReminderService.js';
import { IAlaskaHunkTrackerService } from '../common/alaskaHunkTracker.js';
import { AlaskaHunkTrackerService } from './alaskaHunkTrackerService.js';
import { IAlaskaPtySessionService } from '../common/alaskaPtySession.js';
import { AlaskaPtySessionService } from './alaskaPtySessionService.js';
...
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration).registerConfiguration({
	id: 'alaska.privacy',
	order: 110,
	title: localize('alaska.privacy.configTitle', 'Xipher IDE Privacy'),
	type: 'object',
	properties: {
...
registerSingleton(IAlaskaGoalService, AlaskaGoalService, InstantiationType.Delayed);
registerSingleton(IAlaskaReminderService, AlaskaReminderService, InstantiationType.Delayed);
registerSingleton(IAlaskaHunkTrackerService, AlaskaHunkTrackerService, InstantiationType.Delayed);
registerSingleton(IAlaskaPtySessionService, AlaskaPtySessionService, InstantiationType.Delayed);
...
}, localize('alaska.tree.readingForeground', 'Color of file labels that Xipher IDE is currently reading.'));
...
class AlaskaAuxBarDefaultContribution extends Disposable implements IWorkbenchContribution {
	...
	private ensureAlaskaInAuxBar(): void {
		...
		// Surface the chat on every startup so it opens with the IDE instead of the user
		// having to hunt for it in the View menu. Deferred a tick so it wins over the
		// workbench's own restore of the last-active aux-bar composite.
		queueMicrotask(() => {
			try {
				this.layoutService.setPartHidden(false, Parts.AUXILIARYBAR_PART);
				this.viewsService.openViewContainer(ALASKA_VIEW_CONTAINER_ID, false);
			} catch (err) {
				this.logService.warn('[alaska.auxBar] auto-open failed', err);
			}
		});
	}
	...
	private handleManualMove(to: ViewContainerLocation): void {
		...
		if (to !== ViewContainerLocation.AuxiliaryBar) {
			...
			this.logService.info(`[alaska.auxBar] user moved Xipher IDE to ${to}, respecting choice`);
		} else {
			this.storageService.remove(ALASKA_AUX_BAR_USER_MOVED_KEY, StorageScope.APPLICATION);
			this.logService.info('[alaska.auxBar] user moved Xipher IDE back to AuxiliaryBar');
		}
	}
}
...
const alaskaIcon = registerIcon('alaska-ai-view-icon', Codicon.sparkle, localize('alaskaViewIcon', 'View icon for the Xipher IDE sidebar.'));

const VIEW_CONTAINER: ViewContainer = Registry.as<IViewContainersRegistry>(ViewContainerExtensions.ViewContainersRegistry).registerViewContainer({
	id: ALASKA_VIEW_CONTAINER_ID,
	title: localize2('alaskaAI', 'Xipher IDE'),
	icon: alaskaIcon,
	hideIfEmpty: false,
	order: 5,
...
Registry.as<IViewsRegistry>(ViewContainerExtensions.ViewsRegistry).registerViews([
	{
		...
		openCommandActionDescriptor: {
			...
			mnemonicTitle: localize({ key: 'miAlaskaAI', comment: ['&& denotes a mnemonic'] }, "&&Xipher IDE"),
			order: 5,
		},
	},
	{
...
class AlaskaSignInAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.signIn', 'Xipher IDE: Sign In'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
...
class AlaskaSignOutAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.signOut', 'Xipher IDE: Sign Out'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
...
class AlaskaPlanApprovalModeAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.plan.approvalMode', 'Xipher IDE: Plan Approval Mode'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
...
class AlaskaSetRunCommandTrustAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.runCommand.trustLevel', 'Xipher IDE: Set Run-Command Trust Level'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const picked = await quickInput.pick(items, { placeHolder: 'Xipher IDE shell-command trust level' });
		if (!picked) { return; }
...
function offerInstallRemoteExtension(notificationService: INotificationService, openerService: IOpenerService, commandService: ICommandService, kind: 'ssh' | 'wsl'): void {
	...
	const handle = notificationService.notify({
		...
		message: localize('alaska.remote.installRequired', 'Xipher IDE нужно расширение {0} для удалённых подключений. Установить сейчас?', kindLabel),
		actions: {
			primary: [
				{
...
class AlaskaConnectSSHAction extends Action2 {
	...
	constructor() {
		super({
			...
			// allow-any-unicode-next-line
			title: localize2('alaska.connectSSH', 'Xipher IDE: Подключиться по SSH'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
			menu: [{
...
class AlaskaConnectWSLAction extends Action2 {
	...
	constructor() {
		super({
			...
			// allow-any-unicode-next-line
			title: localize2('alaska.connectWSL', 'Xipher IDE: Открыть в WSL'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
			menu: [{
				id: MenuId.StatusBarRemoteIndicatorMenu,
...
class AlaskaOpenRemoteMenuAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.openRemoteMenu', 'Xipher IDE: Open Remote Menu'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
...
class AlaskaAcceptAllPendingAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.acceptAllPending', 'Xipher IDE: Accept All Pending Edits'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
			keybinding: {
...
class AlaskaRevertAllPendingAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.revertAllPending', 'Xipher IDE: Revert All Pending Edits'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
...
class AlaskaResetChatLocationAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.resetChatLocation', 'Xipher IDE: Reset Chat to Right Sidebar'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
...
class AlaskaSwitchToPlanModeAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.agentMode.switchToPlan', 'Xipher IDE: Switch Agent to Plan Mode'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
			keybinding: {
				weight: KeybindingWeight.WorkbenchContrib,
...
class AlaskaSwitchToActModeAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.agentMode.switchToAct', 'Xipher IDE: Switch Agent to Act Mode'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
...
class AlaskaApprovePlanAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.agentMode.approvePlan', 'Xipher IDE: Approve Plan and Switch to Act'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
			keybinding: {
				weight: KeybindingWeight.WorkbenchContrib,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (!ok) {
			accessor.get(INotificationService).warn(localize('alaska.agentMode.approvePlan.empty', 'No draft tasklist to approve yet. Ask Xipher IDE to propose a plan first.'));
		}
	}
}
...
class AlaskaDumpMetricsAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.dumpMetrics', 'Xipher IDE: Dump Performance Metrics to Log'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		svc.dumpToLog();
		notify.info(localize('alaska.dumpMetrics.done', 'Xipher IDE metrics dumped to log — open Output → Log (Window) to inspect.'));
	}
}
...
class AlaskaRegenerateTitleAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.regenTitle', 'Xipher IDE: Regenerate Session Title'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
...
class AlaskaExportSessionMarkdownAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.exportSessionMd', 'Xipher IDE: Export Session as Markdown'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
...
class AlaskaExportSessionJsonAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.exportSessionJson', 'Xipher IDE: Export Session as JSON'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(AlaskaSettingsEditor, AlaskaSettingsEditor.ID, localize('alaska.settings.paneLabel', 'Xipher IDE Settings')),
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: { value: 'Xipher IDE: Open Settings', original: 'Xipher IDE: Open Settings' },
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
...
class AlaskaIndexStatusbarContribution extends Disposable implements IWorkbenchContribution {
	...
	private renderEntry(): { name: string; text: string; ariaLabel: string; tooltip: string; command?: string; showProgress?: boolean | 'loading' | 'syncing' } {
		...
		let icon = '$(database)';
		let label = localize('alaska.index.statusbar.idle', 'Xipher Index');
		let showProgress: boolean | 'loading' | 'syncing' = false;
		switch (p.state) {
			case 'disabled':
				icon = '$(circle-slash)';
				label = localize('alaska.index.statusbar.disabled', 'Xipher Index off');
				break;
			case 'unauthenticated':
				icon = '$(key)';
				label = localize('alaska.index.statusbar.signedOut', 'Xipher Index: sign in');
				break;
			case 'scanning':
				icon = '$(search)';
			...
			case 'error':
				icon = '$(warning)';
				label = localize('alaska.index.statusbar.error', 'Xipher Index error');
				break;
			case 'idle':
			default:
				icon = '$(database)';
				if (p.chunkCount > 0) {
					label = localize('alaska.index.statusbar.synced', 'Xipher Index: {0} chunks', p.chunkCount.toLocaleString());
				}
				break;
		}
		...
		return {
			name: localize('alaska.index.statusbar.name', 'Xipher Index'),
			text,
			ariaLabel: label,
			tooltip,
...
function renderIndexTooltip(p: { state: string; filesIndexed: number; filesTotal: number; chunkCount: number; bytesUsed: number; lastIndexedAt?: number; errorMessage?: string }): string {
	const lines: string[] = [];
	lines.push(localize('alaska.index.tooltip.title', 'Xipher IDE — Codebase Index'));
...
class AlaskaIndexReindexAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.index.reindex', 'Xipher IDE: Reindex Workspace'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		try {
			...
			notify.info(localize('alaska.index.reindex.done', 'Xipher IDE reindex finished — {0} chunks stored.', index.progress.chunkCount.toLocaleString()));
		} catch (err) {
			notify.error(localize('alaska.index.reindex.failed', 'Xipher IDE reindex failed: {0}', err instanceof Error ? err.message : String(err)));
		} finally {
			source.dispose();
		}
	}
...
class AlaskaIndexClearAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.index.clear', 'Xipher IDE: Clear Workspace Index'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		try {
			...
			notify.info(localize('alaska.index.clear.done', 'Xipher IDE workspace index cleared.'));
		} catch (err) {
			notify.error(localize('alaska.index.clear.failed', 'Xipher IDE clear failed: {0}', err instanceof Error ? err.message : String(err)));
		}
	}
}
...
class AlaskaIndexToggleAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('alaska.index.toggle', 'Xipher IDE: Toggle Codebase Index'),
			category: { value: 'Xipher IDE', original: 'Xipher IDE' },
			f1: true,
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		notify.info(!enabled
			? localize('alaska.index.toggle.on', 'Xipher IDE index enabled — reload to (re)start indexing.')
			: localize('alaska.index.toggle.off', 'Xipher IDE index disabled. Chats fall back to file-tree snapshot until re-enabled.'));
	}
}
