...
export class FileBasedRecommendations extends ExtensionRecommendations {
	...
	private onModelAdded(model: ITextModel): void {
		const uri = model.uri.scheme === Schemas.alaskacodeNotebookCell ? CellUri.parse(model.uri)?.notebook : model.uri;
		if (!uri) {
			return;
		}

		const supportedSchemes = distinct([Schemas.untitled, Schemas.file, Schemas.alaskacodeRemote, ...this.workspaceContextService.getWorkspace().folders.map(folder => folder.uri.scheme)]);
