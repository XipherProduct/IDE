	...
	override async run(): Promise<any> {
		...
		if (this.extension.deprecationInfo) {
			...
			if (this.extension.deprecationInfo.extension) {
				...
				detail = localize('deprecated with alternate settings message', "This extension is deprecated as this functionality is now built-in to Alaska AI.");
...
export class UninstallAction extends ExtensionAction {
	...
	override async run(): Promise<any> {
		...
		try {
			...
			alert(localize('uninstallExtensionComplete', "Please reload Alaska AI to complete the uninstallation of the extension {0}.", this.extension.displayName));
		} catch (error) {
	...
	private async computeAndUpdateStatus(): Promise<void> {
		...
		if (this.extension.deprecationInfo) {
			if (this.extension.deprecationInfo.extension) {
				...
				this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize('deprecated with alternate settings tooltip', "This extension is deprecated as this functionality is now built-in to Alaska AI. Configure these {0} to use this functionality.", link)) }, true);
			} else {
