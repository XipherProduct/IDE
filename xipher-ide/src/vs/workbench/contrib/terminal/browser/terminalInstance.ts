	...
	private async _onProcessExit(exitCodeOrError?: number | ITerminalLaunchError): Promise<void> {
		...
		if (waitOnExit && this._processManager.processState !== ProcessState.KilledByUser) {
			...
			if (exitMessage) {
				...
				const suppressForFeature = !failedDuringLaunch && this._shellLaunchConfig.isFeatureTerminal === true;
				if (!suppressForFeature && (failedDuringLaunch || (this._terminalConfigurationService.config.showExitAlert && this.xterm?.lastInputEvent !== /*Ctrl+D*/'\x04'))) {
