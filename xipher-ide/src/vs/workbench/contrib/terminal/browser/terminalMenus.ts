...
export function setupTerminalMenus(): void {
	...
	MenuRegistry.appendMenuItem(MenuId.EditorTitleContext, {
		...
		when: ResourceContextKey.Scheme.isEqualTo(Schemas.alaskacodeTerminal),
		group: '2_files'
	});
	...
	for (const menuId of [MenuId.EditorTitle, MenuId.CompactWindowEditorTitle]) {
		...
		MenuRegistry.appendMenuItem(menuId, {
			...
			group: 'navigation',
			order: 9,
			when: ContextKeyExpr.and(ResourceContextKey.Scheme.isEqualTo(Schemas.alaskacodeTerminal), TerminalContextKeys.terminalDictationInProgress.negate()),
			isHiddenByDefault: true
		});
		MenuRegistry.appendMenuItem(menuId, {
			command: {
			...
			group: 'navigation',
			order: 10,
			when: ContextKeyExpr.and(ResourceContextKey.Scheme.isEqualTo(Schemas.alaskacodeTerminal), HasSpeechProvider, TerminalContextKeys.terminalDictationInProgress),
			isHiddenByDefault: true
		});
	}
}
