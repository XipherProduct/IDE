...
export class NotebookMultiTextDiffEditor extends EditorPane {
	...
	public getDiffElementViewModel(uri: URI): IDiffElementViewModelBase | undefined {
		if (uri.scheme === Schemas.alaskacodeNotebookCellOutput || uri.scheme === Schemas.alaskacodeNotebookCellOutputDiff ||
			uri.scheme === Schemas.alaskacodeNotebookCellMetadata || uri.scheme === Schemas.alaskacodeNotebookCellMetadataDiff
		) {
		...
		if (uri.scheme === Schemas.alaskacodeNotebookMetadata) {
		...
		return {
			setUri(uri, options = {}) {
				if (!uri) {
					...
					if (uri.scheme === Schemas.alaskacodeNotebookCell) {
						const notebookDocument = uri.scheme === Schemas.alaskacodeNotebookCell ? that.notebookDocumentService.getNotebook(uri) : undefined;
						const cellIndex = Schemas.alaskacodeNotebookCell ? that.notebookDocumentService.getNotebook(uri)?.getCellIndex(uri) : undefined;
					...
					} else if (uri.scheme === Schemas.alaskacodeNotebookCellMetadata || uri.scheme === Schemas.alaskacodeNotebookCellMetadataDiff) {
						description = localize('notebookCellMetadataLabel', "Metadata");
					} else if (uri.scheme === Schemas.alaskacodeNotebookCellOutput || uri.scheme === Schemas.alaskacodeNotebookCellOutputDiff) {
