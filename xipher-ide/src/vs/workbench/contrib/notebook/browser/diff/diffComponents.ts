	...
	private async _buildMetadataEditor() {
		...
		if (this.cell instanceof SideBySideDiffElementViewModel) {
			...
			const [originalMetadataModel, modifiedMetadataModel] = await Promise.all([
				this.textModelService.createModelReference(CellUri.generateCellPropertyUri(this.cell.originalDocument.uri, this.cell.original.handle, Schemas.alaskacodeNotebookCellMetadata)),
				this.textModelService.createModelReference(CellUri.generateCellPropertyUri(this.cell.modifiedDocument.uri, this.cell.modified.handle, Schemas.alaskacodeNotebookCellMetadata))
			]);
			...
			const modelUri = CellUri.generateCellPropertyUri(uri, handle, Schemas.alaskacodeNotebookCellMetadata);
