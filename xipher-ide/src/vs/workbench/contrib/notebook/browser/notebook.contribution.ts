...
class CellInfoContentProvider {
	...
	constructor(
		...
		this._disposables.push(textModelService.registerTextModelContentProvider(Schemas.alaskacodeNotebookCellMetadata, {
			provideTextContent: this.provideMetadataTextContent.bind(this)
		}));

		this._disposables.push(textModelService.registerTextModelContentProvider(Schemas.alaskacodeNotebookCellOutput, {
			provideTextContent: this.provideOutputTextContent.bind(this)
		}));

		this._disposables.push(this._labelService.registerFormatter({
			scheme: Schemas.alaskacodeNotebookCellMetadata,
			formatting: {
				label: '${path} (metadata)',
				separator: '/'
			}
		...
		this._disposables.push(this._labelService.registerFormatter({
			scheme: Schemas.alaskacodeNotebookCellOutput,
			formatting: {
				label: '${path} (output)',
				separator: '/'
			}
	...
	async provideMetadataTextContent(resource: URI): Promise<ITextModel | null> {
		...
		const data = CellUri.parseCellPropertyUri(resource, Schemas.alaskacodeNotebookCellMetadata);
		if (!data) {
			return null;
		}
	...
	async provideOutputsTextContent(resource: URI): Promise<ITextModel | null> {
		...
		const data = CellUri.parseCellPropertyUri(resource, Schemas.alaskacodeNotebookCellOutput);
		if (!data) {
			return null;
		}
...
class NotebookMetadataContentProvider {
	...
	constructor(
		...
		this._disposables.push(textModelService.registerTextModelContentProvider(Schemas.alaskacodeNotebookMetadata, {
			provideTextContent: this.provideMetadataTextContent.bind(this)
		}));

		this._disposables.push(this._labelService.registerFormatter({
			scheme: Schemas.alaskacodeNotebookMetadata,
			formatting: {
				label: '${path} (metadata)',
				separator: '/'
			}
