...
export class BackLayerWebView<T extends ICommonCellInfo> extends Themable {
	...
	private asWebviewUri(uri: URI, fromExtension: URI | undefined) {
		return asWebviewUri(uri, fromExtension?.scheme === Schemas.alaskacodeRemote ? { isRemote: true, authority: fromExtension.authority } : undefined);
	}
		...
		this._register(this.webview.onMessage(async (message) => {
			...
			switch (data.type) {
				...
				case 'clicked-link': {
					...
					} else if (matchesScheme(data.href, Schemas.alaskacodeNotebookCell)) {
						const uri = URI.parse(data.href);
