...
import { AlaskaMcpMainService } from '../../platform/alaskaMcp/electron-main/alaskaMcpMainService.js';
import { IAlaskaWslService, ALASKA_WSL_CHANNEL } from '../../platform/alaskaWsl/common/alaskaWsl.js';
import { AlaskaWslMainService } from '../../platform/alaskaWsl/electron-main/alaskaWslMainService.js';
import { IAlaskaHookHostService, ALASKA_HOOK_CHANNEL } from '../../platform/alaskaHooks/common/alaskaHooks.js';
import { AlaskaHookHostService } from '../../platform/alaskaHooks/electron-main/alaskaHookHostService.js';
import { IAlaskaUpdateMainService, ALASKA_UPDATE_CHANNEL } from '../../platform/alaskaUpdate/common/alaskaUpdate.js';
import { AlaskaUpdateMainService } from '../../platform/alaskaUpdate/electron-main/alaskaUpdateMainService.js';
...
export class CodeApplication extends Disposable {
	...
	private static readonly SECURITY_PROTOCOL_HANDLING_CONFIRMATION_SETTING_KEY = {
		...
		[Schemas.alaskacodeRemote]: 'security.promptForRemoteFileProtocolHandling' as const
	};
	...
	private configureSession(): void {
		...
		const isUrlFromWindow = (requestingUrl?: string | undefined) => requestingUrl?.startsWith(`${Schemas.alaskacodeFileResource}://${VSCODE_AUTHORITY}`);
		const isUrlFromWebview = (requestingUrl: string | undefined) => requestingUrl?.startsWith(`${Schemas.alaskacodeWebview}://`);
		...
		const supportedSvgSchemes = new Set([Schemas.file, Schemas.alaskacodeFileResource, Schemas.alaskacodeRemoteResource, Schemas.alaskacodeManagedRemoteResource, 'devtools']);
		...
		const isSafeFrame = (requestFrame: WebFrameMain | null | undefined): boolean => {
			for (let frame: WebFrameMain | null | undefined = requestFrame; frame; frame = frame.parent) {
				if (frame.url.startsWith(`${Schemas.alaskacodeWebview}://`)) {
					return true;
				}
			}
			return false;
		...
		session.defaultSession.webRequest.onBeforeRequest((details, callback) => {
			...
			if (uri.scheme === Schemas.alaskacodeWebview) {
		...
		session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
			...
			if (contentTypes && Array.isArray(contentTypes)) {
				...
				if (!uri.path.endsWith(Schemas.alaskacodeRemoteResource) && contentTypes.some(contentType => contentType.toLowerCase().includes('image/svg'))) {
					return callback({ cancel: !isSvgRequestFromSafeContext(details) });
				}
			}
	...
	private registerListeners(): void {
		...
		app.on('web-contents-created', (event, contents) => {
			...
			if (contents?.opener?.url.startsWith(`${Schemas.alaskacodeFileResource}://${VSCODE_AUTHORITY}/`)) {
	...
	async startup(): Promise<void> {
		this.logService.debug('Starting Alaska AI');
	...
	private setupManagedRemoteResourceUrlHandler(mainProcessElectronServer: ElectronIPCServer) {
		...
		protocol.registerBufferProtocol(Schemas.alaskacodeManagedRemoteResource, (request, callback) => {
	...
	private async shouldBlockOpenable(openable: IWindowOpenable, windowsMainService: IWindowsMainService, dialogMainService: IDialogMainService): Promise<boolean> {
		...
		if (openableUri.scheme !== Schemas.file && openableUri.scheme !== Schemas.alaskacodeRemote) {
	...
	private getWindowOpenableFromProtocolUrl(uri: URI): IWindowOpenable | undefined {
		...
		// Remote path
		else if (uri.authority === Schemas.alaskacodeRemote) {
			...
			const remoteUri = URI.from({ scheme: Schemas.alaskacodeRemote, authority, path, query, fragment: uri.fragment });
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// Alaska MCP host (spawns external MCP servers from main process)
		services.set(IAlaskaMcpService, new SyncDescriptor(AlaskaMcpMainService, undefined, false));

		// Alaska WSL host (spawns wsl.exe for distro discovery on Windows)
		services.set(IAlaskaWslService, new SyncDescriptor(AlaskaWslMainService, undefined, false));
		services.set(IAlaskaHookHostService, new SyncDescriptor(AlaskaHookHostService, undefined, false));

		// Alaska self-updater (downloads + swaps AppImage / runs installer on quit)
		services.set(IAlaskaUpdateMainService, new SyncDescriptor(AlaskaUpdateMainService, undefined, false));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		// Alaska MCP host channel
		const alaskaMcpService = accessor.get(IAlaskaMcpService);
		const alaskaMcpChannel = ProxyChannel.fromService(alaskaMcpService, disposables);
		mainProcessElectronServer.registerChannel(ALASKA_MCP_CHANNEL, alaskaMcpChannel);

		// Alaska WSL host channel
		const alaskaWslService = accessor.get(IAlaskaWslService);
		const alaskaWslChannel = ProxyChannel.fromService(alaskaWslService, disposables);
		mainProcessElectronServer.registerChannel(ALASKA_WSL_CHANNEL, alaskaWslChannel);

		// Alaska hooks host channel
		const alaskaHookHostService = accessor.get(IAlaskaHookHostService);
		const alaskaHookHostChannel = ProxyChannel.fromService(alaskaHookHostService, disposables);
		mainProcessElectronServer.registerChannel(ALASKA_HOOK_CHANNEL, alaskaHookHostChannel);

		// Alaska updater channel
		const alaskaUpdateService = accessor.get(IAlaskaUpdateMainService);
		const alaskaUpdateChannel = ProxyChannel.fromService(alaskaUpdateService, disposables);
		mainProcessElectronServer.registerChannel(ALASKA_UPDATE_CHANNEL, alaskaUpdateChannel);
	...
	private afterWindowOpen(instantiationService: IInstantiationService): void {
		...
		// Remote Authorities
		protocol.registerHttpProtocol(Schemas.alaskacodeRemoteResource, (request, callback) => {
			callback({
				url: request.url.replace(/^alaskacode-remote-resource:/, 'http:'),
