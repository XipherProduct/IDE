...
class WordHighlighter {
	...
	constructor(
		...
		this.toUnhook.add(editor.onDidBlurEditorWidget(() => {
			...
			if (!activeEditor) { // clicked into nb cell list, outline, terminal, etc
				this._stopAll();
			} else if (activeEditor.getModel()?.uri.scheme === Schemas.alaskacodeNotebookCell && this.editor.getModel()?.uri.scheme !== Schemas.alaskacodeNotebookCell) { // switched tabs from non-nb to nb
				this._stopAll();
			}
		}));
	...
	private _stopSingular(): void {
		...
		if (this.editor.hasTextFocus()) {
			if (this.editor.getModel()?.uri.scheme !== Schemas.alaskacodeNotebookCell && WordHighlighter.query?.modelInfo?.modelURI.scheme !== Schemas.alaskacodeNotebookCell) { // clear query if focused non-nb editor
	...
	private getOtherModelsToHighlight(model: ITextModel): ITextModel[] {
		...
		// notebook case
		const isNotebookEditor = model.uri.scheme === Schemas.alaskacodeNotebookCell;
		if (isNotebookEditor) {
			const currentModels: ITextModel[] = [];
			const currentEditors = this.codeEditorService.listCodeEditors();
			for (const editor of currentEditors) {
				const tempModel = editor.getModel();
				if (tempModel && tempModel !== model && tempModel.uri.scheme === Schemas.alaskacodeNotebookCell) {
	...
	private async _run(multiFileConfigChange?: boolean, delay?: number): Promise<void> {
		...
		} else if (this.model.uri.scheme === Schemas.alaskacodeNotebookCell) {
	...
	constructor(
		...
		this._register(editor.onDidChangeModel((e) => {
			if (this._wordHighlighter) {
				if (!e.newModelUrl && e.oldModelUrl?.scheme !== Schemas.alaskacodeNotebookCell) { // happens when switching tabs to a notebook that has focus in the cell list, no new model URI (this also doesn't make it to the wordHighlighter, bc no editor.hasModel)
