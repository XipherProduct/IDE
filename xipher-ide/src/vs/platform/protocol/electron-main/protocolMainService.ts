...
export class ProtocolMainService extends Disposable implements IProtocolMainService {
	...
	private handleProtocols(): void {
		...
		defaultSession.protocol.registerFileProtocol(Schemas.alaskacodeFileResource, (request, callback) => this.handleResourceRequest(request, callback));
		...
		this._register(toDisposable(() => {
			defaultSession.protocol.unregisterProtocol(Schemas.alaskacodeFileResource);
	...
	private handleResourceRequest(request: Electron.ProtocolRequest, callback: ProtocolCallback): void {
		...
		this.logService.error(`${Schemas.alaskacodeFileResource}: Refused to load resource ${path} from ${Schemas.alaskacodeFileResource}: protocol (original URL: ${request.url})`);
