...
export class AlaskaInlineDiffContribution extends Disposable implements IEditorContribution {
	...
	private renderRemoved(removed: readonly IRemovedBlock[]): void {
		...
		this.editor.changeViewZones(accessor => {
			for (const block of removed) {
				...
				dom.setAttribute('aria-label', localize('alaska.diff.removed.aria', 'Removed by Alaska AI'));
	...
	private renderToolbar(diff: ILineDiff): void {
		...
		summary.textContent = localize('alaska.diff.summary', 'Alaska AI edit · +{0} / −{1}', this.addedCount, this.removedCount);
...
class ApproveDiffAction extends EditorAction {
	constructor() {
		super({
			...
			label: localize2('alaska.diff.approve.action', 'Alaska AI: Approve Inline Diff'),
...
class DiscardDiffAction extends EditorAction {
	constructor() {
		super({
			...
			label: localize2('alaska.diff.discard.action', 'Alaska AI: Discard Inline Diff'),
			precondition: ALASKA_DIFF_ACTIVE,
			kbOpts: {
				kbExpr: ALASKA_DIFF_ACTIVE,
...
class NextDiffFileAction extends EditorAction {
	constructor() {
		super({
			...
			label: localize2('alaska.diff.nextFile.action', 'Alaska AI: Next Pending Diff File'),
...
class PrevDiffFileAction extends EditorAction {
	constructor() {
		super({
			...
			label: localize2('alaska.diff.prevFile.action', 'Alaska AI: Previous Pending Diff File'),
			precondition: EditorContextKeys.editorTextFocus,
			kbOpts: {
				kbExpr: EditorContextKeys.editorTextFocus,
