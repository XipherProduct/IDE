...
export const TOOL_DEFINITIONS: readonly IAlaskaTool[] = [
	...
	},
	{
		...
		function: {
			...
			description:
				'Replace one exact, unique snippet inside an existing file. Use only when the snippet is short and you are sure it appears once. For anything bigger, prefer alaska_write_file with the full new content.',
			parameters: {
				type: 'object',
				properties: {
					path: { type: 'string', description: 'Workspace-relative path of the file to patch.' },
					find: { type: 'string', description: 'Exact snippet to find. Must occur exactly once in the file.' },
	...
	},
	{
		...
		function: {
			...
			description:
				'Read the contents of a workspace file. Call this BEFORE patching so your find-string matches the current bytes exactly. Returns at most 64 KiB; larger files are truncated and flagged.',
			parameters: {
				type: 'object',
				properties: { path: { type: 'string', description: 'Workspace-relative path.' } },
				required: ['path'],
...
const RUN_WAIT_ON_EXIT = (exitCode: number) => `\r\n[Alaska AI] Command finished with exit code ${exitCode}. The terminal is kept open so you can scroll back; close this tab when you're done.\r\n`;
...
export class AlaskaToolExecutor {
	...
	private async patchFile(call: IAlaskaToolCall): Promise<IAlaskaToolResult> {
		const args = this.parseArgs<{ path?: unknown; find?: unknown; replace?: unknown }>(call);
		const path = ensureString(args.path, 'path');
		...
		if (first < 0) {
			return this.softFail(call, 'find-string not present in file — read it first and match exact bytes including whitespace', 'find_missed');
		}
		const second = current.indexOf(find, first + find.length);
		if (second >= 0) {
			return this.fail(call, 'find-string is ambiguous (appears more than once) — narrow it down or use alaska_write_file with the full new content');
		}
		const next = current.slice(0, first) + replace + current.slice(first + find.length);
		return {
			callId: call.id,
			name: call.name,
			content: JSON.stringify({ ok: true, action: 'patch', path, bytes: bytesOf(next) }),
	...
	private async signedPost(path: string, body: string, call: IAlaskaToolCall): Promise<{ res: Response; body: string } | { softFail: IAlaskaToolResult }> {
		...
		if (!creds || !userId) {
			return { softFail: this.softFail(call, 'not signed in — run "Alaska AI: Sign In" from the command palette', 'unauthorized') };
		}
