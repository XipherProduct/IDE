...
export class AlaskaChatViewPane extends ViewPane {
	...
	private buildSignedOut(parent: HTMLElement): HTMLElement {
		...
		heading.textContent = 'Welcome to Alaska AI';
		root.appendChild(heading);

		const blurb = document.createElement('p');
		blurb.textContent = 'Sign in with your Alaska AI account to chat with the assistant about the code you have open.';
		root.appendChild(blurb);

		const btn = document.createElement('button');
		btn.className = 'alaska-button';
		btn.textContent = 'Sign in to Alaska AI';
	...
	private async startSignIn(): Promise<void> {
		...
		try {
			...
			this.signInButton.textContent = 'Sign in to Alaska AI';
		}
	}
	...
	private buildSignedIn(parent: HTMLElement): HTMLElement {
		...
		leftActions.className = 'alaska-composer-bar-left';
		leftActions.appendChild(this.buildAttachMenu());
		leftActions.appendChild(this.buildActiveContextToggle());
		leftActions.appendChild(this.buildPermissionsMenu());
		leftActions.appendChild(this.buildAgentModePicker());
		bar.appendChild(leftActions);

		const rightActions = document.createElement('div');
		rightActions.className = 'alaska-composer-bar-right';
		rightActions.appendChild(this.buildModelPicker());
		rightActions.appendChild(this.buildEffortPicker());
		rightActions.appendChild(this.buildAutoContinueToggle());
		rightActions.appendChild(this.buildTokenCounter());

		const status = document.createElement('span');
		status.className = 'alaska-stream-status';
		status.setAttribute('aria-live', 'polite');
		status.setAttribute('aria-atomic', 'true');
		this.statusEl = status;
		rightActions.appendChild(status);

		const stopBtn = document.createElement('button');
		stopBtn.type = 'button';
		stopBtn.className = 'alaska-cbtn alaska-cbtn-stop';
		// allow-any-unicode-next-line
		stopBtn.textContent = '■';
		stopBtn.title = localize('alaska.composer.stop', 'Stop generating');
	...
	private updateModelPicker(): void {
		...
		for (const model of this.models) {
			const item = document.createElement('button');
			item.className = `alaska-model-item${model.id === selected.id ? ' alaska-model-item-active' : ''}`;
			item.type = 'button';
			...
			plan.textContent = model.requires_plan;
	...
	private buildMoreMenuAction(): HTMLElement {
		...
		const populate = (): void => {
			...
			group.textContent = localize('alaska.chat.more.heading', 'Alaska AI');
	...
	private refreshModeBanner(): void {
		...
		if (mode === 'plan') {
			...
			if (draft && draft.trim()) {
				...
				hint.textContent = localize('alaska.mode.banner.planHint', 'Ask Alaska AI to explore the codebase. When it finishes, it will propose a numbered tasklist for your approval.');
	...
	private renderPlanApprovalCard(parent: HTMLElement, sessionId: string, draftMarkdown: string): void {
		...
		approveBtn.addEventListener('click', () => {
			...
			if (!ok) {
				this.notificationService.warn(localize('alaska.plan.parseFailed', 'Could not parse a numbered tasklist from the response. Ask Alaska AI to format the steps as a numbered list (1., 2., ...).'));
			}
		});
	...
	private renderSettingsOverlay(): void {
		...
		title.textContent = 'Alaska AI Settings';
	...
	private buildSlashContext(): IAlaskaSlashContext {
		...
		this.instantiationService.invokeFunction(a => { accessor = a; });
		if (!accessor) {
			throw new Error('Failed to obtain services accessor for slash command');
		}
	...
	private renderThreadMessage(msg: IThreadMessage): HTMLElement {
		...
		roleStrong.textContent = msg.role === 'assistant'
			? localize('alaska.chat.author.alaska', 'ALASKA')
			: msg.role === 'error'
	...
	private maybeOpenMentionPicker(): void {
		const ta = this.composerEl;
		if (!ta) { return; }
	...
	private async openMentionPicker(): Promise<void> {
		type KindItem = IQuickPickItem & { kind: AlaskaMentionKind };
		const items: KindItem[] = [
			{ kind: 'file', label: '$(file) File', description: 'Pin one workspace file' },
			{ kind: 'folder', label: '$(folder) Folder', description: 'Pin every file under a workspace folder (cap 50)' },
			{ kind: 'url', label: '$(link-external) URL', description: 'Fetch and pin a web page' },
			{ kind: 'diag', label: '$(warning) Diagnostics', description: 'Pin current LSP markers from the active file' },
		];
		const picked = await this.quickInputService.pick(items, {
			placeHolder: 'Pin context for the next prompt — file / folder / url / diag',
		});
		if (!picked) {
			this.composerEl?.focus();
			return;
		...
		switch (picked.kind) {
			case 'file': await this.pickFileMention(); break;
			case 'folder': await this.pickFolderMention(); break;
			case 'url': await this.pickUrlMention(); break;
			case 'diag': this.addDiagMention(); break;
	...
	private addDiagMention(): void {
		const ctx = this.contextSvc.captureCurrent();
		if (!ctx.diagnostics || ctx.diagnostics.length === 0) {
			this.notificationService.notify({ severity: Severity.Info, message: 'No diagnostics in the active file to pin.' });
			return;
		}
		this.addMention({ kind: 'diag', target: 'active-file', label: `@diag (${ctx.diagnostics.length})` });
	}
	...
	private async pickImageAttachment(): Promise<void> {
		const picked = await this.fileDialogService.showOpenDialog({
			...
			title: localize('alaska.attach.imageDialog', 'Attach images for Alaska AI'),
		});
	...
	private showPendingPlaceholder(refs: ILiveAssistantRefs): void {
		...
		const label = document.createElement('span');
		label.textContent = localize('alaska.chat.pending', 'Alaska AI thinking…');
		wrap.appendChild(label);
	...
	private async openLiveDiff(callId: string, relPath: string): Promise<ILiveWriteSession | undefined> {
		...
		try {
			await this.editorService.openEditor({
				label: `Alaska AI · writing ${relPath}…`,
				description: '',
	...
	private async handleRunCommand(call: IAlaskaToolCall, msg: IThreadMessage, refs: ILiveAssistantRefs, executor: AlaskaToolExecutor | undefined): Promise<IAlaskaToolResult> {
		...
		try {
			...
			const terminalName = `Alaska AI · ${command.length > 40 ? command.slice(0, 40) + '…' : command}`;
	...
	private async driveRunCommandTerminal(call: IAlaskaToolCall, msg: IThreadMessage, refs: ILiveAssistantRefs, activity: IThreadToolActivity, config: import('../../../../platform/terminal/common/terminal.js').IShellLaunchConfig): Promise<IAlaskaToolResult> {
		...
		let abortPhase: 'none' | 'soft' | 'hard' | 'dispose' = 'none';
	...
	private async assessRunCommandConsent(command: string): Promise<{ granted: true } | { granted: false; reason: string; errorCode: string }> {
		...
		const picked = await this.quickInputService.pick(items, {
			placeHolder: `Alaska AI wants to run: ${head}`,
			matchOnDescription: false,
			matchOnDetail: false,
			ignoreFocusLost: true,
			title: 'Alaska AI · Shell command',
	...
	private renderPlanCard(msg: IThreadMessage, refs: ILiveAssistantRefs, activity: IThreadToolActivity): Promise<{ approved: boolean; summary: string; toolPayload: Record<string, unknown>; activityPatch: Partial<IPlanActivity> }> {
		return new Promise((resolve) => {
			...
			headerText.textContent = `Alaska AI proposes ${activity.plan!.changes.length} file change${activity.plan!.changes.length === 1 ? '' : 's'}`;
			...
			const saveEdit = () => {
				...
				editor.style.display = 'none';
				list.style.display = '';
				headerText.textContent = `Alaska AI proposes ${newChanges.length} file change${newChanges.length === 1 ? '' : 's'} (edited)`;
	...
	private async applyUserMessageEdit(idx: number, newContent: string): Promise<void> {
		...
		if (this.composerEl) {
			this.composerEl.value = newContent;
			this.thread.pop();
			void this.submit();
		}
	}
	...
	private setStreaming(streaming: boolean): void {
		...
		if (this.stopButton) {
			this.stopButton.style.display = streaming ? '' : 'none';
		}
		if (this.statusEl) {
...
function runCardFooterLabel(rc: IRunCommandActivity): string {
	switch (rc.status) {
		...
		case 'blocked':
			return rc.reason ? `Blocked: ${rc.reason}` : 'Blocked by Alaska AI policy';
	}
}
...
function describeEmptyTurn(
	...
	if (calls.length === 0) {
		return 'No response from Alaska AI.';
	}
	...
	if (reads === calls.length && reads > 0) {
		// allow-any-unicode-next-line
		return `Alaska AI read ${reads} file${reads === 1 ? '' : 's'} but didn't follow up with any edit — reply with \`продолжи\` or rephrase the request.`;
	}
	// allow-any-unicode-next-line
	return `Alaska AI ran ${calls.length} tool call${calls.length === 1 ? '' : 's'} but didn't produce a final answer — reply with \`продолжи\` so it finishes the turn.`;
}

function isThreadToolActivity(value: unknown): value is IThreadToolActivity {
	if (!value || typeof value !== 'object') { return false; }
	const v = value as Partial<IThreadToolActivity>;
	return typeof v.id === 'string'
		&& typeof v.name === 'string'
		&& (v.status === 'running' || v.status === 'ok' || v.status === 'err' || v.status === 'skipped');
}
...
// allow-any-unicode-next-line
// allow-any-unicode-next-line
function shouldInferFileOperation(prompt: string): boolean {
	const en = /\b(create|make|build|add|write|generate|implement|refactor|fix|update|rewrite|scaffold)\b/i.test(prompt);
	// allow-any-unicode-next-line
	const ru = /(созда|сдела|добав|напиш|сгенер|запил|сверст|реализ|верст|обнов|перепиш|собе?ри|сваргань|накодь|нагенер)/i.test(prompt);
	return en || ru;
}

function inferEditOperationsFromResponse(prompt: string, content: string): IAlaskaEditOperation[] {
	const blocks = extractCodeBlocks(content);
	if (blocks.length === 0) {
		return [];
	}
	const assistantWantsToWrite = /\b(here is|here's|created|made|saved|built|generated|wrote|added|implemented)\b/i.test(content)
		// allow-any-unicode-next-line
		|| /(сделал|создал|сохранил|написал|собрал|сгенерировал|реализовал|добавил|готов)/i.test(content);
	if (!shouldInferFileOperation(prompt) && !assistantWantsToWrite) {
		return [];
	}
	...
	for (const block of blocks) {
		const annotated = extractAnnotatedPath(block.code);
		const path = annotated ?? inferTargetPath(prompt, block.language);
		if (!path || used.has(path)) {
			continue;
		}
		used.add(path);
		const body = annotated ? stripFirstLine(block.code) : block.code;
		ops.push({ action: 'create', path, content: body.trimEnd() + '\n' });
	}
	return ops;
}

function extractAnnotatedPath(code: string): string | undefined {
	const firstLine = code.split('\n', 1)[0] ?? '';
	const trimmed = firstLine.trim();
	const m =
		trimmed.match(/^(?:\/\/|#)\s*([a-zA-Z0-9_./\-]+\.[a-zA-Z0-9]{1,8})\s*$/) ||
		trimmed.match(/^\/\*\s*([a-zA-Z0-9_./\-]+\.[a-zA-Z0-9]{1,8})\s*\*\/$/) ||
		trimmed.match(/^<!--\s*([a-zA-Z0-9_./\-]+\.[a-zA-Z0-9]{1,8})\s*-->$/);
	if (!m) {
		return undefined;
...
function stripFirstLine(code: string): string {
	const idx = code.indexOf('\n');
	return idx >= 0 ? code.slice(idx + 1) : '';
}
...
function inferTargetPath(prompt: string, language: string): string | undefined {
	const explicit = prompt.match(/(?:^|[\s"'`])([a-zA-Z0-9_.\-\/]+\.(?:html|css|js|jsx|ts|tsx|json|md|py|go|rs|java|kt|cpp|c|h|cs|php|rb|swift|vue|svelte))(?:$|[\s"'`,.!?])/);
	if (explicit) {
