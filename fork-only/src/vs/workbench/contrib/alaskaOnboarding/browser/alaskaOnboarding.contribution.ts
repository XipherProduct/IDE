...
class AlaskaOnboardingContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		walkthroughService.registerWalkthrough({
			...
			title: localize('alaska.onboarding.title', 'Welcome to Alaska AI'),
			description: localize('alaska.onboarding.description', 'Connect your account, pick a model, and try the chat — 4 quick steps.'),
			order: -1,
			source: 'Alaska AI',
			isFeatured: true,
			when: trueExpr,
			walkthroughPageTitle: localize('alaska.onboarding.pageTitle', 'Alaska AI Onboarding'),
			icon: { type: 'icon', icon: ThemeIcon.fromId(Codicon.sparkle.id) },
			steps: [
				{
					id: 'alaska.onboarding.connect',
					title: localize('alaska.onboarding.connect.title', 'Connect your Alaska AI account'),
					...
					media: { type: 'svg', altText: 'Alaska AI', path: FileAccess.asFileUri('vs/workbench/contrib/welcomeGettingStarted/common/media/ai-powered-suggestions.svg') },
				},
				{
				...
				},
				{
					...
					title: localize('alaska.onboarding.settings.title', 'Tune Alaska AI settings'),
					description: localize('alaska.onboarding.settings.desc', 'Permissions, plan approval mode, run-command trust — all live in the dedicated settings editor. [Open Alaska settings](command:alaska.chat.openSettings)'),
					category: 'alaska.onboarding',
					when: trueExpr,
					order: 4,
